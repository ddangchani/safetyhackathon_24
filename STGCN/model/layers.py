...
class Align(nn.Module):
    """
    Align the input tensor to the output tensor by padding or convolution.
    """
    ...
    def forward(self, x):
        if self.c_in > self.c_out: # if the input channel is larger than the output channel, use convolution to align the channel
            x = self.align_conv(x)
        elif self.c_in < self.c_out: # if the input channel is smaller than the output channel, use zero padding to align the channel
...
class CausalConv1d(nn.Conv1d):
    ...
    def forward(self, input):
        result = super(CausalConv1d, self).forward(input) # call the forward function of the parent class (nn.Conv1d)
        if self.__padding != 0:
            return result[: , : , : -self.__padding]
        
        return result # dimension: [batch_size, out_channels, timesteps]
